...
namespace Nacos.V2.Remote
{
    ...
    public abstract class RpcClient : IDisposable
    {
        ...
        private volatile TaskCompletionSource<Dictionary<string, bool>> _abilityTcs = new TaskCompletionSource<Dictionary<string, bool>>();
        private Dictionary<string, bool> _serverAbilityTable;

        protected void ResetAbilityNegotiation()
            => _abilityTcs = new TaskCompletionSource<Dictionary<string, bool>>();

        protected void CompleteAbilityNegotiation(Dictionary<string, bool> abilityTable)
        {
            _serverAbilityTable = abilityTable ?? new Dictionary<string, bool>();
            _abilityTcs.TrySetResult(_serverAbilityTable);
        }

        protected bool WaitForAbilityNegotiation(int timeoutMs = 3000)
            => _abilityTcs.Task.Wait(timeoutMs);

        public bool IsAbilitySupported(string abilityKey)
            => _serverAbilityTable?.TryGetValue(abilityKey, out var v) == true && v;
