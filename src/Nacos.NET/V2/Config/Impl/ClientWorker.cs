namespace Nacos.V2.Config.Impl
{
    ...
    public class ClientWorker : IDisposable
    {
        ...
        {
            if (_agent is ConfigRpcTransportClient rpc)
                return rpc.FuzzyWatchConfig(group, dataIdPattern, watcher);

            _logger?.LogWarning("[ClientWorker] FuzzyWatchConfig requires gRPC transport (ConfigUseRpc=true)");
            return Task.CompletedTask;
        }

        public Task FuzzyUnwatchConfig(string group, string dataIdPattern, IConfigFuzzyWatcher watcher)
        {
            if (_agent is ConfigRpcTransportClient rpc)
                return rpc.FuzzyUnwatchConfig(group, dataIdPattern, watcher);
