namespace Nacos.V2.Config.Impl
{
    ...
    public class ConfigRpcTransportClient : AbstConfigTransportClient
    {
        ...
        private ConfigFuzzyWatchNotifyRequestHandler _fuzzyWatchHandler;
            ...
            ConcurrentDictionary<string, CacheData> cacheMap)
        {
            ...
            this._fuzzyWatchHandler = new ConfigFuzzyWatchNotifyRequestHandler(logger);
        ...
        private RpcClient EnsureRpcClient(string taskId)
        {
            lock (_obj)
            {
                ...
                RpcClient rpcClient = RpcClientFactory
                        .CreateClient($"{uuid}_config-{taskId}", RemoteConnectionType.GRPC, newlabels, _options.TLSConfig, _options.GrpcKeepalive);
        ...
        private void InitHandlerRpcClient(RpcClient rpcClientInner)
        {
            ...
            rpcClientInner.RegisterServerPushResponseHandler(_fuzzyWatchHandler);
            rpcClientInner.RegisterConnectionListener(new ConfigRpcConnectionEventListener(_logger, rpcClientInner, _cacheMap, _listenExecutebell, RedoFuzzyWatchesAsync));
        ...
        private string ResourceBuild(CommonRequest request)
        {
            ...
            if (request is ConfigFuzzyWatchRequest cfwReq)
                return GetResource(cfwReq.Namespace, cfwReq.Group);
        ...
        private async Task RefreshContentAndCheck(CacheData cacheData, bool notify)
        {
            try
            {
                ...
                var encKey = resp.GetEncryptedDataKey();
                if (encKey.IsNotNullOrWhiteSpace())
                    cacheData.EncryptedDataKey = encKey;
        ...
        public async Task FuzzyWatchConfig(string group, string dataIdPattern, IConfigFuzzyWatcher watcher)
        {
            var rpcClient = GetOneRunningClient();

            if (!rpcClient.IsAbilitySupported(AbilityKeys.ConfigFuzzyWatch))
            {
                _logger?.LogWarning("[CONFIG-FUZZY-WATCH] Server does not support ConfigFuzzyWatch ability, skip");
                return;
            }

            var request = new ConfigFuzzyWatchRequest
            {
                Namespace = GetTenantInner(),
                Group = group,
            ...
            var timeOut = _options.DefaultTimeOut > 0 ? _options.DefaultTimeOut : 3000;
            var response = await RequestProxy(rpcClient, request, timeOut).ConfigureAwait(false);

            if (response == null || !response.IsSuccess())
                _logger?.LogWarning("[CONFIG-FUZZY-WATCH] FuzzyWatchConfig request failed: {0}", response?.Message);
            else
                _fuzzyWatchHandler.AddWatcher(group, dataIdPattern, watcher);
        }

        public async Task FuzzyUnwatchConfig(string group, string dataIdPattern, IConfigFuzzyWatcher watcher)
        {
            var rpcClient = GetOneRunningClient();
            ...
            if (response == null || !response.IsSuccess())
                _logger?.LogWarning("[CONFIG-FUZZY-UNWATCH] FuzzyUnwatchConfig request failed: {0}", response?.Message);
            else
                _fuzzyWatchHandler.RemoveWatcher(group, dataIdPattern, watcher);
        }

        private async Task RedoFuzzyWatchesAsync()
        {
            var rpcClient = GetOneRunningClient();
            var timeOut = _options.DefaultTimeOut > 0 ? _options.DefaultTimeOut : 3000;

            foreach (var (group, pattern) in _fuzzyWatchHandler.GetWatchedPatterns())
            {
                try
                {
                    var request = new ConfigFuzzyWatchRequest
                    ...
                    await RequestProxy(rpcClient, request, timeOut).ConfigureAwait(false);
                    _logger?.LogInformation("[CONFIG-FUZZY-WATCH] Redo subscribe group={0} pattern={1}", group, pattern);
                }
                catch (Exception ex)
                {
